...
subtitle: "Between and Within-Subject t-Tests Demos"
...
A one-sample *t*-test was performed to evaluate whether there was a difference between the weight in a sample of mice (*n* = 10) compared to the general population of mice ($\mu = 25$). The mean weight in the sample of mice (M = 20.14, SD = 1.90) was statistically significantly different than the average weight of mice in the general population, *t*(9) = -8.11, *p* < .001, Cohen's *d* = -2.56, 95% CI [-3.87, -1.23].
...
stat_test <- mice2_long  %>%
  t_test(weight ~ group, 
         ref.group = "before", 
         paired = TRUE, 
         alternative = "two.sided") %>% # `alternative =` can be set to `greater` or `less` as well
  add_significance() %>% 
  add_xy_position(x = "group")

# Print boxplots with significance bracket

bxp + 
  stat_pvalue_manual(stat_test, tip.length = .01)
```
...
# Add p-value and significance levels
stat_test <- dep_long  %>%
  t_test(depression ~ time_period, 
         paired = TRUE, 
         ref.group = "t0", 
         alternative = "greater") %>%
  add_significance() %>% 
  add_xy_position(x = "time_period")
...
An independent samples *t*-test was conducted to determine whether there was a difference between the extra hours of sleep obtained by participants who received the experimental medication and participants who received the placebo. The results indicated that participants who received the medication (*M* = 2.33, *SD* = 2.00) did not have statistically significantly greater levels of extra hours of sleep compared to participants who received the placebo (*M* = 0.75, *SD* = 1.79), *t*(18) = -1.86, *p* = .079, Cohen's *d* = -0.83, 95% CI [-1.739, 0.096].