...
subtitle: "Week 11 - Chi-Square Demo"
...
**Null hypothesis**: $H_0 = \text{Equal Proportions Between Doctoral and Master}$ --- Program level will be equally distributed across our sample of $n = 1000$ participants, $p_{doctoral} = p_{masters}$

**Alternative hypothesis**: $H_A = \text{Unequal Proportions}$ --- Program level will be unequally distributed across our sample of $n = 1000$ participants, $p_{doctoral} \neq p_{masters}$
...
A chi-square ($\chi^2$) goodness of fit test was performed to evaluate whether the distribution of professors was equivalent across `doctoral` and `masters` programs. The distribution of professors was different between `doctoral` and `masters` programs,  $\chi^2(1, N = 1000) = 396.90, p < .001$. Observed and expected frequencies for each condition are presented in Table X.
...
**Null hypothesis**: $H_0 = \text{Proportion Between Doctoral and Master Equivalent to .80/.20}$ --- Program level will be distributed with 80% Doctoral and 20% Masters across our sample of $n = 1000$ participants, $p_{doctoral} = .80; p_{masters} = .20$

**Alternative hypothesis**: $H_A = \text{Different Proportional Breakdown}$ --- Program level will not be distributed with 80% Doctoral and 20% Masters across our sample of $n = 1000$ participants, $p_{doctoral} \neq .80; p_{masters} \neq .20$
...
A chi-square ($\chi^2$) goodness of fit test was performed to evaluate whether the distribution of professors across `doctoral` and `masters` programs was 80% doctoral/20% masters, respectively. The distribution of professors across `doctoral` and `masters` programs was not different than expected (80%/20%),  $\chi^2(1, N = 1000) = 1.41, p =  .24$. Observed and expected frequencies for each condition are presented in Table X.
...
**Null hypothesis**: $H_0 = \text{Equal Proportions Between Female and Male Professors}$ --- Gender will be equally distributed across our $n = 1000$ sample, $p_{male} = p_{female}$

**Alternative hypothesis**: $H_A = \text{Unequal Proportions}$ --- Gender will be unequally distributed across our $n = 1000$ sample, $p_{male} \neq p_{female}$
```{r}
propTestN(data = datA,
               var = 'sex',
               expected = TRUE, 
...
A chi-square ($\chi^2$) goodness of fit test was performed to evaluate whether the distribution of `gender` was equivalent across `males` and `female` professors. The distribution of `gender` was different between `male` and `female` professors,  $\chi^2(1, N = 1000) = 34.60, p < .001$. Observed and expected frequencies for each condition are presented in Table X.
...
**Null hypothesis**: $H_0 = \text{Equal Proportions Amongst Rank Levels}$ --- Professor rank will be equally distributed across our $n = 1000$ sample, $p_{Full} = p_{Associate} = p_{Assistant} = p_{Instructor}$

**Alternative hypothesis**: $H_A = \text{Unequal Proportions}$ --- Professor rank will not be equally distributed across our $n = 1000$ sample, $p_{Full} \neq p_{Associate} \neq p_{Assistant} \neq p_{Instructor}$
```{r}
propTestN(data = datA,
               var = 'rank',
               expected = TRUE, 
...
A chi-square ($\chi^2$) goodness of fit test was performed to evaluate whether the distribution of the `rank` of sampled professors was equivalent across `Full Professor`, `Associate Professor`, `Assistant Professor` and `Instructors`. The distribution of `rank` was different among the levels of professor `rank`,  $\chi^2(3, N = 1000) = 341.88, p < .001$. Observed and expected frequencies for each condition are presented in Table X.
...
**Null hypothesis**: $H_0 = \text{Observed Proportions Amongst Rank Levels Approximate Expected}$ --- Professor rank will be distributed with 42% Full, 28% Associate, 28% Assistant, and 2% Instructors across our sample of $n = 1000$ participants, $p_{Full} = .42; p_{Associate} = .28; p_{Assistant} = .28; p_{Instructor} = .02$

**Alternative hypothesis**: $H_A = \text{Observed Proportions Different Than Expected}$ --- Professor rank will not be distributed with 42% Full, 28% Associate, 28% Assistant, and 2% Instructors across our sample of $n = 1000$ participants, $p_{Full} \neq .42; p_{Associate} \neq .28; p_{Assistant} \neq .28; p_{Instructor} \neq .02$
...
A chi-square ($\chi^2$) goodness of fit test was performed to evaluate whether the distribution of professors across `Full`, `Associate`, `Assistant`, and `Instructor` ranks was 42% `Full`, 28% `Associate`, 28% `Assistant` and 2% `Instructor`. The distribution of `rank` was not different than expected,  $\chi^2(3, N = 1000) = 0.52, p =  .91$. Observed and expected frequencies for each condition are presented in Table X.
...
**Write Up/Interpretation**: A chi-square ($\chi^2$) test of independence was performed to evaluate the relationship between `gender` and `program level` of professor. These variables were not associated, $\chi^2(1, N = 1000) = 0.08, p = .78, \phi = .01$. Neither `males` nor `females` were more likely to be employed in the `doctoral` or `masters` programs than expected.  
...
**Write Up/Interpretation**: A chi-square ($\chi^2$) test of independence was performed to evaluate the relationship between `gender` and `professor rank`. These variables were associated, $\chi^2(3, N = 1000) = 41.89, p <.001, \text{Cramer's V} = .21$. `Professors` were more frequently `male` in this sample. Furthermore, more `male` professors were employed as `full professor` and more `female` professors were employed as `assistant` and `associate professor` than expected. `Cramer's V` values of this magnitude are considered weak (i.e., small) as an effect size measure of association between `gender` and `rank`. 
...
**Write Up/Interpretation**: A chi-square ($\chi^2$) test of independence was performed to evaluate the relationship between `program level` and `professor rank`. These variables were not associated, $\chi^2(3, N = 1000) = 4.91, p = .18, \text{Cramer's V} = .07$. `Cramer's V` values of this magnitude are considered weak as an effect size measure of association between `program level` and `rank`. There was no association among graduate program and professor rank in this sample of data.
...
null_dist_P <- datA %>%
  ...
  calculate(stat = "z")
# `type = "draw"` is ONLY used for this chi-square GoF test
...
null_dist_datA %>%
  get_p_value(obs_stat = chi_sq$stat, direction="greater")
...
null_dist_2 %>%
  get_p_value(obs_stat = chi_sq2$stat, direction="greater")
...
null_dist_3 %>%
  get_p_value(obs_stat = chi_sq3$stat, direction="greater")

# OR 

pchisq(chi_sq3$stat, 3, lower.tail = FALSE)
```
**The main difference between the `bootstrap` and `permute` functions in the infer pipeline**: 

- `bootstrapping` --- best for estimating confidence intervals
- `permuting` --- best for testing hypotheses
